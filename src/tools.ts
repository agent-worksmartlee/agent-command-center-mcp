...
export async function registerAgent(client: AgentCommandClient, input: z.infer<typeof RegisterAgentSchema>) {
  // Check if already registered
  const existing = await client.get<unknown[]>("agents", {
    "name": `eq.${input.name}`,
...
export async function getAgents(client: AgentCommandClient, input: z.infer<typeof GetAgentsSchema>) {
  const query: Record<string, string> = { select: "id,name,status,agent_type,capabilities" };
  if (input.status) query["status"] = `eq.${input.status}`;
  return client.get<unknown[]>("agents", query);
}

export async function updateAgentStatus(client: AgentCommandClient, input: z.infer<typeof UpdateAgentStatusSchema>) {
  return client.patch("agents", input.agent_id, { status: input.status });
}

export async function getProjects(client: AgentCommandClient, input: z.infer<typeof GetProjectsSchema>) {
  const query: Record<string, string> = { select: "id,name,status,description" };
  if (input.status) query["status"] = `eq.${input.status}`;
  return client.get<unknown[]>("projects", query);
}

export async function createProject(client: AgentCommandClient, input: z.infer<typeof CreateProjectSchema>) {
...
export async function getTasks(client: AgentCommandClient, input: z.infer<typeof GetTasksSchema>) {
  const query: Record<string, string> = {
    select: "id,title,description,steps,status,autonomous_level,priority,assigned_agent_id,project_id,result,created_at",
    order: "priority.asc",
  };
  if (input.status) query["status"] = `eq.${input.status}`;
  if (input.project_id) query["project_id"] = `eq.${input.project_id}`;
  if (input.assigned_agent_id) query["assigned_agent_id"] = `eq.${input.assigned_agent_id}`;
  if (input.unassigned) query["assigned_agent_id"] = "is.null";
  return client.get<unknown[]>("tasks", query);
}

export async function createTask(client: AgentCommandClient, input: z.infer<typeof CreateTaskSchema>) {
...
export async function claimTask(client: AgentCommandClient, input: z.infer<typeof ClaimTaskSchema>) {
...
export async function completeTask(client: AgentCommandClient, input: z.infer<typeof CompleteTaskSchema>) {
  return client.patch("tasks", input.task_id, {
    status: "completed",
...
export async function failTask(client: AgentCommandClient, input: z.infer<typeof FailTaskSchema>) {
  return client.patch("tasks", input.task_id, {
    status: "failed",
...
export async function createEscalation(client: AgentCommandClient, input: z.infer<typeof CreateEscalationSchema>) {
...
export async function getEscalations(client: AgentCommandClient, input: z.infer<typeof GetEscalationsSchema>) {
  const query: Record<string, string> = { select: "id,task_id,agent_id,reason,status,created_at" };
  if (input.status) query["status"] = `eq.${input.status}`;
  if (input.task_id) query["task_id"] = `eq.${input.task_id}`;
  return client.get<unknown[]>("escalations", query);
}

export async function resolveEscalation(client: AgentCommandClient, input: z.infer<typeof ResolveEscalationSchema>) {
...
export async function logActivity(client: AgentCommandClient, input: z.infer<typeof LogActivitySchema>) {
